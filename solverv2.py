...
def eliminate(poss,x,y,val):
    possible=copy.deepcopy(poss)
    for i in range(9):
        possible[x][i]&=(1<<10)-(1<<val)-2
    for i in range(9):
        possible[i][y]&=(1<<10)-(1<<val)-2

    top=(x)//3
    left=(y)//3
    for i in range(3):
        for j in range(3):
            possible[3*top+i][3*left+j]&=(1<<10)-(1<<val)-2
    
    return possible

def dfs(sudoku,poss,coord):
    if (coord==81):
        return (True,sudoku)
    if (sudoku[coord//9][coord%9]==0):
        for x in range(1,10):
            if ((1<<x)&poss[coord//9][coord%9]!=0):
                temp_sudoku=copy.deepcopy(sudoku)
                row=coord//9
                col=coord%9
                temp_sudoku[row][col]=x
                if (check_vert(temp_sudoku,col) and check_hori(temp_sudoku,row) and check_box(temp_sudoku,coord)):
                    temp_poss=eliminate(poss,row,col,x)
                    TEMP=dfs(temp_sudoku,temp_poss,coord+1)
                    if (TEMP[0]): return TEMP
        return (False,None)
    else: return dfs(sudoku,poss,coord+1)


def func(sudoku):
    poss=[[(1<<10)-2 for x in range(9)] for y in range(9)]
    for x in range(9):
        for y in range(9):
            if (sudoku[x][y]!=0):
                poss=eliminate(poss,x,y,sudoku[x][y])
    return dfs(sudoku,poss,0)
